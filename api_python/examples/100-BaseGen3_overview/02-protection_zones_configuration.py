...
PROTECTION_ZONE_POS =  [0.5, -0.3, 0.5]

# Size of the protection zone (in meters)
PROTECTION_ZONE_DIMENSIONS = [0.3, 0.3, 0.3]

# Theta values of the protection zone movement (in degrees)
PROTECTION_ZONE_MOVEMENT_THETAS = [10.0, 90.0, 10.0 ]
...
def move_to_protection_zone(base):
    ...
    cartesian_pose.x = PROTECTION_ZONE_POS[0]     # (meters)
...
def print_protection_zones(base):
    ...
    for protection_zone in all_protection_zones.protection_zones:
        ...
        " Origin : [ " \
        + protection_zone.shape.origin.x.str() + " "\
        + protection_zone.shape.origin.y.str() + " "\
        + protection_zone.shape.origin.z.str() \
        + " ] Dimensions : [ "
